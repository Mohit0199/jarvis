...
def extract_yt_term(command):
    # Remove 'on youtube' from the command if it exists
    command = re.sub(r'\s+on\s+youtube', '', command, flags=re.IGNORECASE)
    # Define a regular expression pattern to capture the song name after 'play'
    pattern = r'play\s+(.*)'
